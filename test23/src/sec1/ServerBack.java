...
public class ServerBack extends Thread {
	...
	ArrayList<String> nickNameList = new ArrayList<>();	// 클라이언트 닉네임 목록
	...
	class ReceiveInfo extends Thread {
		...
		public ReceiveInfo(Socket socket) {
			try {
				...
				nickName = in.readUTF();	// 인코딩해서 nickName에 넣음
		...
		public void run() {
			try {
				serverChatGui.userList.setText(null);
				serverChatGui.appendUserList(nickNameList);
				transmitAll(nickName + "님이 입장 하셨습니다.\n");
				
				for(int i=0; i<nickNameList.size(); i++) {
					transmitAll("+++닉네임의시작+++" + nickNameList.get(i));
				}
				serverChatGui.appendMessage(nickName + "님이 입장 하셨습니다.\n");
				while(true) {
					message = in.readUTF();
				...
				transmitAll(message + "님이 퇴장하셨습니다.");
				for(int i=0; i<nickNameList.size(); i++) {
